...
const state = {
    stops: [
        { id: generateId(), color: '#00ff88', position: 0 },
        { id: generateId(), color: '#0055ff', position: 100 }
    ],
    direction: 'r',
    type: 'linear',
    flipX: false,
...
function bindStopEvents() {
    ...
    pickers.forEach(p => p.addEventListener('input', (e) => updateStopColor(e.target.dataset.id, e.target.value)));
    hexInputs.forEach(h => h.addEventListener('change', (e) => {
        let val = e.target.value;
        if (!val.startsWith('#')) val = '#' + val;
        if (/^#[0-9A-F]{6}$/i.test(val)) {
            updateStopColor(e.target.dataset.id, val);
        } else {
            e.target.value = state.stops.find(s => s.id === e.target.dataset.id).color;
        }
    }));
    sliders.forEach(s => s.addEventListener('input', (e) => updateStopPosition(e.target.dataset.id, parseInt(e.target.value))));
    
    removes.forEach(btn => btn.addEventListener('click', (e) => removeStop(e.target.dataset.id)));
}
...
function addStop(pos = null, color = null) {
    const newPos = pos !== null ? pos : (state.stops.length > 0 ? Math.min(100, state.stops[state.stops.length - 1].position + 10) : 50);
...
const linearCssMap = {
    ...
    'b': 'to bottom', 'bl': 'to bottom left', 'l': 'to left', 'tl': 'to top left', 'c': 'to right'
};

const radialCssMap = {
    't': 'at top', 'tr': 'at top right', 'r': 'at right', 'br': 'at bottom right',
    'b': 'at bottom', 'bl': 'at bottom left', 'l': 'at left', 'tl': 'at top left', 'c': 'at center'
};

const linearTwMap = {
    't': 'bg-gradient-to-t', 'tr': 'bg-gradient-to-tr', 'r': 'bg-gradient-to-r', 'br': 'bg-gradient-to-br',
    'b': 'bg-gradient-to-b', 'bl': 'bg-gradient-to-bl', 'l': 'bg-gradient-to-l', 'tl': 'bg-gradient-to-tl', 'c': 'bg-gradient-to-r'
};
...
function generateTailwind() {
    ...
    if (state.type === 'linear' && count <= 3) {
        ...
        const stopsStr = sortedStops.map(s => `${s.color}_${s.position}%`).join(',_');
        if (state.type === 'linear') {
...
function updateOutput() {
    ...
    els.previewArea.style.cssText = css;

    els.previewBadge.textContent = tw.length > 40 ? tw.substring(0, 40) + '...' : tw;
...
els.copyClassBtn.addEventListener('click', (e) => copyText(generateTailwind(), e.target));
els.copyCssBtn.addEventListener('click', (e) => copyText(generateCss(), e.target));
els.copyTwCodeBtn.addEventListener('click', (e) => copyText(generateTailwind(), e.target));
els.copyCssCodeBtn.addEventListener('click', (e) => copyText(generateCss(), e.target));
